...
pal1<- leaflet::colorFactor(palette = "Spectral", domain = values(test_2016), na.color = "transparent")
pal2 <- leaflet::colorFactor(palette = "Spectral", domain = values(test_2014), na.color = "transparent")
pal3 <- leaflet::colorFactor(palette = "Spectral", domain = values(test_2012), na.color = "transparent")
pal4 <- leaflet::colorFactor(palette = "Spectral", domain = values(test_2010), na.color = "transparent")
pal5 <- leaflet::colorFactor(palette = "Spectral", domain = values(test_2008), na.color = "transparent")
pal6 <- leaflet::colorFactor(palette = "Spectral", domain = values(test_2001), na.color = "transparent")
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
      pal <- colorFactor(rainbow(229), domain = values(test_2016)) %>%
      ...
      addMarkers(data = parks, icon = leafIcons, group = "Parks", popup = ~(FEATURE_NAME)) %>%
        ...
        addRasterImage(test_2016, pal1, opacity = 1, group = "Land Cover 2016") %>%
        addRasterImage(test_2014, pal2, opacity = 1, group = "Land Cover 2014") %>%
        addRasterImage(test_2012, pal3, opacity = 1, group = "Land Cover 2012") %>%
        addRasterImage(test_2010, pal4, opacity = 1, group = "Land Cover 2010") %>%
        addRasterImage(test_2008, pal5, opacity = 1, group = "Land Cover 2008") %>%
        addRasterImage(test_2001, pal6, opacity = 1, group = "Land Cover 2001")
    ...
    proxy <- leafletProxy("map", data = filteredData()) %>%
      ...
      addPolygons(label = ~paste ("Tract:", tract, paste0(input$variable,":"),as.numeric(eval(as.symbol(input$variable)))),
                  ...
                  highlightOptions = highlightOptions(color = "white",
                                                      weight = 2,
                                                      bringToFront = TRUE)) %>%
      addLegend(pal = pal4, opacity = 1, values = values(test_2010), labFormat = function(type, cuts, p) {paste0(labels2)}, group = "Land Cover Legend", title = "Land Cover Legend") %>%
