...
for (i in years.list){
  ...
  #uncomment this code if you want to rename the numeric column names with their descriptive names         
  #    if(sum(is.na(df[1,]) != 0)){
  #      n <- df[which(is.na(df[1,]))]
  #      m <- colnames(n)
  #      df[which(is.na(df[1,]))] <- m
  #    }
  #    sums[i,j] <- sum(is.na(df[1,]))
  #    names(df) <- as.vector(df[1,]) 
  #    df <- df %>% select(unique(names(df)))
...
With the DP02 table constructed, we can construct similiar tables using the automative technique developed for DP02. It is important to note the discrepancies in the 'percent' column and discontinuity between 2010 ACS survey data and other years in DP02 - that will have to be addressed later.
